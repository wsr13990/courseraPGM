...
for i=1:numPeople
  if pedigree.parents(i,:) == [0,0]
    factorList(i).var = i;
    factorList(i).card = 3*ones(length(factorList(i).var),1);
    factorList(i).val = alleleFreqs';
    factorList(numPeople+i).var = numPeople+i;
    factorList(numPeople+i).card = 3*ones(length(factorList(numPeople+i).var),1);
    factorList(numPeople+i).val = alleleFreqs';
  else
    #factorList(i) = genotypeGivenParentsGenotypesFactor(
    #  2, i, pedigree.parents(i,1), numPeople+pedigree.parents(i,1)
    #);
    factorList(i) = childCopyGivenParentalsFactor(
      numAlleles,i,pedigree.parents(i,1),numPeople+pedigree.parents(i,1)
    );
    factorList(numPeople+i) = childCopyGivenParentalsFactor(
      numAlleles,numPeople+i,pedigree.parents(i,2),numPeople+pedigree.parents(i,2)
    );
    #factorList(numPeople+i) = genotypeGivenParentsGenotypesFactor(
    #  2, numPeople+i, pedigree.parents(i,2), numPeople+pedigree.parents(i,2)
    #);
  endif
  factorList(2*numPeople+i) = phenotypeGivenCopiesFactor(
    alphaList, numAlleles, i, numPeople+i, 2*numPeople+i
  );
endfor
