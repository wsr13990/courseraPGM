...
genotypeFactor.card = length(genotypesToAlleles).*ones(1,3);

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
assignments = IndexToAssignment(1:prod(genotypeFactor.card), genotypeFactor.card);
for i=1:length(assignments)
  genotypeFactor.val(i) = sum(sum(genotypesToAlleles(assignments(i,2),:)'*genotypesToAlleles(assignments(i,3),:)==prod(genotypesToAlleles(assignments(i,1),:))))/4;
endfor
